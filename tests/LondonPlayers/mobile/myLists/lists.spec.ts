...
async function writeNumberInInput(page: Page, number: string) {
  const input = page.locator('#my-favorite-numbers-detail #inputNumber-mobile');
...
async function saveList(page: Page) {
  const saveButton = page.locator(
    '#my-favorite-numbers-detail .container-save-favorite-mobile img.btn-save-favorite[src*="save-my-list-btn.svg"]'
  );
...
test.describe('Lotto Player - My Lists (Pick 5)', () => {
  ...
  test('should edit numbers and name in an existing Pick 5 list', async ({
    ...
    await writeListName(page, 'Test #2 Edit List');
  ...
  test('should select all numbers from a complete Pick 5 list and play', async ({
    page,
  }) => {
