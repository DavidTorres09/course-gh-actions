...
test.describe('Lotto Player Profile End-to-End', () => {
  ...
  test('should submit negative feedback using a preset message', async ({
    page,
  }) => {
  ...
  test('should set favorite pick mode from carousel and reflect in selection', async ({
    page,
  }) => {
    ...
    const frontPickLocator = page.locator('#profile-pick-options-carousel img.carousel-center');
    const pickOption = await frontPickLocator.getAttribute('data-pick-option');

    await acceptChangesModal(page);

    if (!pickOption) throw new Error('pickOption is undefined');
    const pick = pickOption.toLowerCase();
    await expect(
      page.locator(
        `#pick-options-carousel img.carousel-center[src="/themes/standardBlue/images/${pick}-rounded.svg"]`
      )
